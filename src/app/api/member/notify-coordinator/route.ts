...
// Also sends a confirmation SMS to the volunteer on signup.
...
export async function POST(request: Request) {
  try {
    ...
    // Also send a confirmation SMS to the volunteer when they sign up
    if (action === 'signup' && memberPhone) {
      const normalizedPhone = memberPhone.replace(/\D/g, '');
      if (normalizedPhone) {
        // Look up event details (time + location) if we have eventId
        let timeInfo = '';
        if (eventId) {
          const { data: ev } = await admin.from('events').select('drop_off_start, drop_off_end, drop_off_location').eq('id', eventId).single();
          if (ev) {
